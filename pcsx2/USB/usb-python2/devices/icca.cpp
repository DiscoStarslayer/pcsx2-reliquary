...
namespace usb_python2
{
	bool acio_icca_device::device_write(std::vector<uint8_t>& packet, std::vector<uint8_t>& outputResponse)
	{
		...
		else if (code == 0x0131 || code == 0x0134 || code == 0x0135)
		{
			...
			int curkey = 0;
			if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr -1][0]))
				curkey |= 16;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][1]))
				curkey |= 1;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][2]))
				curkey |= 5;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][3]))
				curkey |= 9;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][4]))
				curkey |= 2;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][5]))
				curkey |= 6;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][6]))
				curkey |= 10;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][7]))
				curkey |= 3;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][8]))
				curkey |= 7;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][9]))
				curkey |= 11;
			else if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][10]))
				curkey |= 4;

			if (Python2Device::GetInputState(usbDev, keypadIdsByDeviceId[header->addr - 1][11]))
			{
